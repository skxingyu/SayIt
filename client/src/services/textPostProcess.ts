...
//
// 提供三类可开关的处理：
//   1. 数字规范化（尽力而为）：百分之 / 小数点 / 分之 / 含位值词的结构化整数 /
 ...
 * 「第 N」序号（第 + 单字数字 + 序数后缀）。
 * 刻意不转的只有「孤立单字数字」（一/二…单独出现，避免拆坏 一起/一点/十分 等词内单字）
 * 与含位值词但无法解析的串。逐位串的转换面向语音报数，书面成语等手打文本不走此路径。
 */
export function convertChineseNumbers(text: string): string {
  ...
  // 4.5 逐位串（无位值词的连续纯数字）：一二三四五 → 12345、一零零二三版本 → 10023版本。
  //     用户语音报数字时逐字读（一零零二三 = 1,0,0,2,3），不是位值记数法，故**逐字映射**
  //     每一位，不能走 parseChineseInteger（那会把「二三」合成 23，丢零）。匹配串限定为
  //     **不含位值词**的纯数字字符（零…九、幺），含 十百千万亿 的串是位值记数，
  //     已由规则 4 正确处理，这里不重复。
  //
  ...
  //    「第」后若是不含位值词的单字数字（一…九、十），此前规则 4 因长度<2 不转，这里补上。
  //    「第两百个」这类含位值词的早已被规则 4 转成「第200个」，无需重复处理，故只取单字数字。
  ...
  //   「一起/一点/十分」这类高频口语词内的单字数字也不该被拆。逐位串（规则 4.5）
  //   用「连续两个以上纯数字」来自我限定，天然避开了单字场景，故无需量词扩展。
