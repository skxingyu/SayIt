...
export function convertChineseNumbers(text: string): string {
  ...
  //     用户语音报数字时逐字读（一零零二三 = 1,0,0,2,3），不是位值记数法，故**逐字映射**一位，
  //     不能走 parseChineseInteger（那会把「二三」合成 23，丢零）。匹配串限定为**不含位值词**的
  //     纯数字字符（零…九、幺），长度 ≥2 —— 含 十百千万亿 的串是位值记数，已由规则 4 正确处理，
  //     这里不重复。成语/惯用语因绝少连续两个纯数字字，几乎天然不受影响（三三两两 → 3322，
  //     属语音报数场景可接受；书面成语手打、不经此路径）。
  result = result.replace(
    new RegExp(`[${DIGIT_CHARS}]{2,}`, 'g'),
    (m: string, offset: number, str: string) => {
      let out = ''
