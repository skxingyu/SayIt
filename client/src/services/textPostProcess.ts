...
//
// 提供三类可开关的处理：
//   1. 数字规范化（尽力而为）：百分之 / 小数点 / 分之 / 时间（X点Y分）/ 小数点分段 /
//      连续逐位串（用户语音逐位报数，如 一零零二三 → 10023）/ 「第 N」序号。
//      **刻意不做**含位值词（十百千万亿）的裸整数换算（三千/三百/二十三 保留中文）——
//      位值词与成语、约数、量词同形，只有带明确格式信号的场域（百分之/分之/点/第）才转。
//   2. 去除句末标点
//   3. 标点符号替换为空格
//
...
/**
 * 中文数字 → 阿拉伯数字（尽力而为）。
 * 覆盖：百分之 / 小数点（数字点数字）/ 分之 / 时间（X点Y分）/ 点分多段（版本号）/
 * 不含位值词的连续逐位串（一二三四五 → 12345，用户语音逐位报数的场景）/
 * 「第 N」序号（第 + 单字数字 + 序数后缀）。
 * **刻意不做**：含位值词（十百千万亿）的裸整数换算（三千/三百/二十五 保留中文）——
 * 位值词与成语、约数、量词同形，只有带明确格式信号的场域（百分之/分之/点/第）才转；
 * 以及孤立单字数字（避免拆坏 一起/一点/十分 等词内单字）。
 * 逐位串的转换面向语音报数，书面成语等手打文本不走此路径。
 */
export function convertChineseNumbers(text: string): string {
  ...
  // 4. 逐位串（不含位值词的连续纯数字）：一二三四五 → 12345、一零零二三版本 → 10023版本。
  //     用户语音报数字时逐字读（一零零二三 = 1,0,0,2,3），不是位值记数法，故**逐字映射**
  //     每一位，不能走 parseChineseInteger（那会把「二三」合成 23，丢零）。
  //
  //     匹配串限定为**不含位值词**的纯数字字符（零…九、幺）。含 十百千万亿 的串
  //     （三千、二十五、一万五）**刻意不转**：位值词与成语（千方百计）、约数（七八个
  //     那类）、量词结构同形，裸换算误伤率不可接受——本 fork 偏好只认带明确格式信号的
  //     场域（百分之/分之/点/第），位值词整数一律保留中文。与上游「结构化整数」规则的
  //     分歧点，rebase 时注意维持删除态。
  //
  //     长度门槛取 ≥3 而不是 ≥2：两字组合在中文里绝大多数**不是**报数，而是约数或副词 ——
  //     「七八个人」≈ 七或八个（改成 78 是量级错误）、「过两三天」、「三五分钟」、
  //     副词「一一说明 / 一一列举」、「二两肉」（重量单位）。这些在日常口述中的频率远高于
  //     两位数报数，而真实的逐位报数几乎都在三位以上（一零零二三、三三零六、一二三四五），
  //     用 ≥3 刚好把两者分开。代价是两位数报数（说「一二」想得 12）不再转 ——
  //     两害相权留下约数正确性。仍会转的已知边界：四字成语「三三两两」→ 3322，
  //     它字面确实连续四个数字字，且口语里罕用到会被误读，故不加黑名单兜。
  result = result.replace(
    new RegExp(`[${DIGIT_CHARS}]{3,}`, 'g'),
    (m: string, offset: number, str: string) => {
      let out = ''
      for (const ch of m) out += String(CN_DIGITS[ch])
      return maybeSpace(str, offset, out)
    },
  )

  // 5. 第 N 序号：第 + 单字数字 + 序数后缀 → 第N + 后缀（第二 → 第2、第三位 → 第3位、第十 → 第10）。
  //    「第」后有明确序数信号才转，是对「单字数字不转」的定点豁免。
  //    「第两百个」这类含位值词的序号不转（位值词整数一律保留中文，见规则 4 注释），
  //    故这里只取单字数字。
  //    必须有明确序数后缀才转，避免把「第一」这类（作强调/列举）误判成序数数字。
  //    「次」刻意不进后缀表：极高频时间副词「第一次/第二次…」（我第一次来北京）与真序数
  //    「第X次获奖」在纯文本上无法区分，还连累「第一次世界大战」这类专名，故宁漏不转。
  const ORD_SUFFIX = '个名位号批轮期章节条目页项'
  result = result.replace(
    new RegExp(`第([一二两三四五六七八九十])([${ORD_SUFFIX}])`, 'g'),
    (m: string, d: string, sfx: string) => {
      const v = parseChineseInteger(d)
      return v === null ? m : `第${v}${sfx}`
    },
  )

  // （量词前的单字数字如 三个 → 3个 不做）单字 + 量词与成语语素在结构上同形
  //   （三个苹果 / 一片狼藉 都是 [单字数字+量词+补足语]），无可靠判别信号；且
  //   「一起/一点/十分」这类高频口语词内的单字数字也不该被拆。逐位串（规则 4）
  //   用「连续三个以上纯数字」来自我限定，天然避开了单字场景，故无需量词扩展。
